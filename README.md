...
AI 工具一个接一个地装进来，用完拍拍屁股就走，缓存、日志、会话、快照和模型却全拉在硬盘里。
更恶心的是，它们藏得还挺深，硬盘快满了，你都不知道是哪一家干的。

AICensus 专门把这些残留揪出来：谁占了空间、拉在哪里、还在不在持续增长，一眼看清。
先找到它们，再一个个收拾。

> AI 工具天天拉，AICensus 天天抓。你的硬盘，该清场了。
